...
void RawPath::moveTo(Vec2D a) {
    const auto n = m_Verbs.size();
    if (n > 0 && m_Verbs[n-1] == PathVerb::move) {
        m_Points[n-1] = a;  // replace previous moveTo position
    } else {
...
void RawPath::lineTo(Vec2D a) {
    m_Points.push_back(a);
    m_Verbs.push_back(PathVerb::line);
}

void RawPath::quadTo(Vec2D a, Vec2D b) {
    m_Points.push_back(a);
    m_Points.push_back(b);
    m_Verbs.push_back(PathVerb::quad);
}

void RawPath::cubicTo(Vec2D a, Vec2D b, Vec2D c) {
    m_Points.push_back(a);
    m_Points.push_back(b);
...
void RawPath::addOval(const AABB& r, PathDirection dir) {
    ...
    if (dir == PathDirection::clockwise) {
        ...
        for (int i = 1; i <= 12; i += 3) {
            cubicTo(map(unit[i+0]), map(unit[i+1]), map(unit[i+2]));
        }
    } else {
        moveTo(map(unit[12]));
...
void RawPath::addPoly(const Vec2D pts[], int count, bool isClosed) {
    if (count <= 0) {
        return;
    }
    ...
    m_Points.reserve(count + isClosed);
    m_Verbs.reserve(count + isClosed);
